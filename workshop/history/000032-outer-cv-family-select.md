...
### 2026-07-13
- 2026-07-13: closed — metis#32 COMPLETE — both milestones boundary-reviewed + all FIX-THEN-SHIP items fixed; all 9 packages GREEN under go test ./... -race (independently re-run in main). M1 (measure/record): ledger Level schema + FamilyEstimate reducer + driver:-drop/config-count-dispatch/--fast + nested run records inner+outer rows (metis#23 sealing preserved); boundary FIX-THEN-SHIP→fixed (1-config degenerate test + stale-driver-docs). M2 (choose/ship): FamilySelect (lowest-SE-within-1-SE, no Ship reuse) + metis select (family from outer rows, config from inner rows, sharp no-outer-rows error, fingerprint join-soundness guard) + select --promote (promotedExperiment reconstruct → all-data ship → best-{family}-{hash}) — retires metis ledger select + metis promote; boundary FIX-THEN-SHIP→fixed (fingerprint-cohort guard + test, datatype-doc field-table parse-trap, dead -m flag). THE HEADLINE: the honest outer-CV estimate now ACTUATES family selection — acceptance gate TestSelect_PicksGeneralizerNotInnerOverfitter proves select --best ships the generalizer over the inner-CV overfitter (the #32 flip; closes the loop metis#23 left open). ACTUAL = N/A (--no-actual): interleaved-session mention-fallback across #20/#31/#32/#33/#34 + fork-executed impl the commit-based measure captures imperfectly. DEFERRED: the kbench peer-repo RUNBOOK rewrite (run->select->submit command model) — a separate peer write.; review verdict: SHIP
- 2026-07-13: closed M2 — M2 (choose/ship side) complete + all 9 packages GREEN under go test ./... -race (independently re-run in main). Delivered: FamilySelect (lowest-SE-within-1-SE, degrades to argmax under one fold, does NOT reuse SweepResult.Ship); metis select (dry) — FAMILY from outer rows (FamilyEstimate+FamilySelect), CONFIG from inner rows (AggregateView+PerFamily+pct-loss), fingerprint-scoped, SHARP ERROR on a multi-family ledger with no outer rows — retires metis ledger select; select --promote reconstructs via promotedExperiment -> all-data ship -> runs/best-{family}-{hash}/submission.csv (empty-ship guard; ship-capture invariant re-homed) — retires metis promote; dead shipWinner deleted; atlas migrated to the run/select/submit model + Level schema. ACCEPTANCE GATE (RED-first): TestSelect_PicksGeneralizerNotInnerOverfitter — inner-CV favors the deep-tree overfitter, the honest OUTER estimate favors the generalizer -> select --best ships the generalizer (the metis#32 flip). Command surface verified: metis select wired; metis promote + ledger select retired. ACTUAL = N/A (--no-actual): interleaved-session mention-fallback across #20/#31/#32/#33/#34 + fork-executed impl the commit-based measure captures imperfectly. DEFERRED: kbench RUNBOOK rewrite (a peer write; command model changed run->select->submit).; review verdict: FIX-THEN-SHIP
