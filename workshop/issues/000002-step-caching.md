...
### 2026-07-05
- 2026-07-05: closed — Close-review round-3 Important (CAS wipeable-cache contract) resolved + reviewer-reproduced-fix verified: cachingExecutor now recomputes (falls through to MISS) on a HIT whose output blob is wiped/corrupt (cas.ErrNotFound/ErrCorrupt) instead of hard-failing — honors pkg/cas consumer contract + design "rm -rf cas/ is safe". TestCache_WipedCASRecomputesNotFails pins it end-to-end. ARCH-DRY upstreamHashes helper. go build+vet+test ./... green. All Critical(0)/Important resolved across 3 close-review rounds; cache soundness independently reviewer-verified + automated (real-D MISS-on-edit, leaf-from-YAML, wipe-recompute). --no-verdict: M3 final milestone reviewed by this close. --no-project: brain tracker by hand.; review verdict: FIX-THEN-SHIP
- 2026-07-05: closed — Close-review round-2 Important (soundness coverage) resolved: TestCachingExecutor_RealDMissesOnSourceEdit (real non-empty D HITs clean, MISSes on a byte-edit to a D file via real gitBlobHashes — locks recompute-only-what-changed, the gap both e2es were blind to) + TestCache_LeafPolicyFromParsedYAML (leaf policy through real experiment.Parse). Minor docs fixed (Entry git-sharing claim, atlas --cache). go build+vet+test ./... green. All Critical(0)/Important across all rounds resolved; cache soundness independently verified by the reviewer + now automated. --no-verdict: M3 final milestone reviewed by this close. --no-project: brain tracker by hand.; review verdict: FIX-THEN-SHIP
- 2026-07-05: closed — Re-close review Important (doc-only) fixed: pkg/record CodeManifest + cmd/metis buildRecord docs corrected — Upstream is populated by buildRecord (K_pre wiring); Code.D/Deps record-provenance deferred to #8 (not "#2 populates", which was stale + self-contradicting above the Upstream-populating code). #2 populates the cache functional Entry.D. go build+vet+test ./... green. All substantive close-review findings resolved (reads.json excluded from output-hash, .metis-cache gitignored, leaf-bypass tested). --no-verdict: M3 final milestone reviewed by this close. --no-project: brain tracker ticked by hand.; review verdict: FIX-THEN-SHIP
- 2026-07-05: closed — Close-review FIX-THEN-SHIP fixes re-reviewed: (1) collectArtifacts excludes reads.json (was leaking into OutputHash -> downstream K_pre, busting downstream on upstream code edits + defeating cross-machine reuse) — test asserts it; (2) .metis-cache/.gitignore written on --cache use (CAS blobs never git-committed). + runner-level immutable-leaf-bypass test, recordMiss/isHit share projectRoot, pkg/cache doc fixed. go build+vet+test ./... green incl -race; cheap-sweeps + real-uv-pipeline e2es still green. --no-verdict: M3 final milestone reviewed by this issue-close. --no-project: brain tracker ticked by hand.; review verdict: FIX-THEN-SHIP
