...
### 2026-07-05
- 2026-07-05: closed — Round-6 close-review Important (both fixed, 0 Critical — doc/test hygiene). (1) Appended ## Revisions to workshop/plans/000008-shape-run-ledger-plan.md (AGENTS.md §1) reconciling delivered semantics: promote commits at HEAD+warns cross-version (not "at the code SHA"), richer promoted_from back-link (point-addr+sweep-SHA+free-param tuple), sidecar written-not-auto-committed, round-4/5 sort semantics. (2) TestLedgerShow_DefaultsDirFromObjective drives the real cmdLedger entrypoint (os.Stdout pipe) with a minimize objective + NO --dir, asserting best-first order; regression-proofed (reverting the round-5 fix flips to worst-first → test fails). go build+vet+test ./... green. All Critical(0)/Important resolved across REWORK + 6 close-review rounds; metis-v1 substrate complete. --no-verdict: M2(REWORK)+M3 folded into this integration close. --no-project: brain tracker by hand.; review verdict: SHIP
- 2026-07-05: closed — Close-review round-5 Important (both fixed): (1) ledger show --sort defaults --dir from the shapes sweep.objective.direction (was hardcoded maximize → minimize sorted worst-first); (2) promote-fidelity tests: TestPromote_WarnsOnCrossCodeVersion (older-winner promote warns to git checkout the sweep-SHA) + round-trip now asserts reproduced METRICS (was point-address only). go build+vet+test ./... green. All Critical(0)/Important resolved across REWORK + 5 close-review rounds; metis-v1 substrate complete. --no-verdict: M2(REWORK)+M3 folded into this close. --no-project: brain tracker by hand.; review verdict: FIX-THEN-SHIP
- 2026-07-05: closed — Close-review round-4 Important (both fixed): (1) ledger show --sort keeps failed/metric-missing rows via ledger.SortAll (was dropping them through TopN; TopN stays for the body summary) — test asserts a failed row survives --sort; (2) captureSweepCode best-effort (warns, does not abort a finished sweep on a git hiccup — records+manifest valid). go build+vet+test ./... green. All Critical(0)/Important resolved across REWORK + 4 close-review rounds. metis-v1 substrate complete. --no-verdict: M2(REWORK)+M3 folded into this close. --no-project: brain tracker by hand.; review verdict: FIX-THEN-SHIP
- 2026-07-05: closed — Close-review round-3 Important (both fixed): (1) ledger show render now testable (renderLedger→io.Writer + header row + blank-pad; showLedger core; TestLedgerShow_RendersSortedTable asserts header + objective-sorted rows both directions — a --sort/--dir regression cannot ship green); (2) namespaced-objective (<step>.<metric>) documented in construct/datatype/experiment-shape.md. go build+vet+test ./... green. All Critical(0)/Important resolved across REWORK + 3 close-review rounds; metis-v1 substrate complete (#9/#3/#2/#6/#7/#8). --no-verdict: M2(REWORK)+M3 folded into this integration close. --no-project: brain tracker by hand (est 3.0/actual 1.95).; review verdict: FIX-THEN-SHIP
...
- **Close-review round-4 Important (both fixed):** (1) **`ledger show --sort` no longer drops failed/metric-missing rows** — was routing through `TopN` (a leaderboard that skips them); added `ledger.SortAll` (sorts qualified by objective, KEEPS the rest appended) for the `show` view (`TopN` stays for the body top-N summary); test asserts a failed row survives `--sort`. (2) **code capture is now best-effort** — a `captureSweepCode` hiccup (no commit identity, read-only object store) no longer hard-fails an already-finished sweep (records + manifest are valid); it warns and forgoes the durable side-ref SHA, consistent with #2's best-effort touch/evict. Full suite green. **Every Critical(0)/Important resolved across REWORK + 4 close-review rounds.**
- **Close-review round-5 Important (both fixed):** (1) **`ledger show --sort` now defaults `--dir` from the shape's objective** — was hardcoded `maximize`, so `--sort` on a `minimize` objective sorted worst-first; now reads `sweep.objective.direction` (explicit `--dir` overrides). (2) **Promote-fidelity tests added** — `TestPromote_WarnsOnCrossCodeVersion` (sweep → advance HEAD → promote --best asserts the "reproduces only after `git checkout <sweep-SHA>`" warning fires — the guard for the committed-at-code-SHA contract); the round-trip test now asserts the re-run reproduces the winning row's **metrics** (was point-address only; Done-when says both). Also widened `promoteOpts.out` to `io.Writer` (testable). All Critical(0)/Important resolved across REWORK + 5 close-review rounds.
- **Close-review round-6 Important (both fixed — doc/test hygiene, 0 Critical):** (1) **plan reconciled with delivered semantics** — appended a `## Revisions` section to `workshop/plans/000008-shape-run-ledger-plan.md` (AGENTS.md §1): promote commits at **HEAD + warns on cross-version** (not "at the code SHA"), the richer `promoted_from` back-link (point-addr + sweep-SHA + free-param tuple), the sidecar written-but-not-auto-committed, and the round-4/5 sort semantics. (2) **round-5 default-`--dir` path now tested** — `TestLedgerShow_DefaultsDirFromObjective` drives the real `cmdLedger` entrypoint (captures `os.Stdout` via a pipe) with a `minimize` objective + NO `--dir`, asserting best-first (lowest-loss) order; regression-proofed by reverting the fix (with it reverted the test fails, worst-first). Full suite green. All Critical(0)/Important resolved across REWORK + 6 close-review rounds; metis-v1 substrate complete.
