...
> **MERGED with Chunk 4 (per operator):** M1a-3 and M1a-4 now close as **ONE review boundary (M1a-3)**. Rationale: keeping the interior cache (Chunk 3) and the `cmd/metis` rewire (Chunk 4) in separate boundaries left `cmd/metis` RED across M1a-3 — so the real-executor soundness gate (Task 13, edit-code→MISS) couldn't run in place. Merging turns `cmd/metis` green *within* the boundary, so the soundness gate runs naturally at close on the real pipeline. Implement Tasks 11–17 together; the boundary closes with **whole-module `go build ./...` green** + the soundness gate + the Titanic run reaching a `(mean,SE)` leaderboard. (The A3 "runs on the existing linear pipeline" note below is now moot — the gate runs on the fully-wired per-fold path.)
...
- **2026-07-08 (M1a-3a shipped, FIX-THEN-SHIP → fixed):** M1a-4 was folded into **M1a-3a** and shipped (whole-module green + the offline Titanic fold sweep → per-config `(mean,SE)` leaderboard). The data-handoff soundness ripple the map under-specified: the per-fold `features→train` dataset handoff had to become a **captured run-dir artifact** (`features` writes to its step dir; `train` reads via `io.dataset_dir`) — an exp-relative shared path is cache-unsound per-fold (a `features` HIT + `train` MISS reads a different fold's data). This rippled into the plain experiments (`titanic-baseline`/`titanic-features`), handled by `train.py`'s dual-mode read. The boundary review (FIX-THEN-SHIP) caught + fixed: a `promote --best` NUL-corruption + unrunnable reconstruction (now **refuses cleanly on a per-fold ledger** — full promote/ship reconstruction is **M1a-5**: it needs the synthesized cv-split + `predict` reading the captured artifact via `io.dataset_dir` — `predict.py` still uses `exp_path`), restored `promote`/`ledger show`/`AggregateView`/`hoistShapePath` test coverage, the `objective.metric` vs emitted-`fold_score` disconnect (fixture + shapes → `train.fold_score`), and the stale `atlas/index.md` `pkg/sweep` bullet (rewritten to `pkg/sampler`). **Open seam for M1a-5:** the driver level is inlined as a bare `Run(GridConfigs)` — `SingleDriver` is built+tested but not in the call path, so #23 (`driver:cv`) must *insert* the outer `Run(driver,…)` wrap; wiring `SingleDriver` in now (even pass-through) would make the three-level algebra real.

- **2026-07-08 (M1a-5 shipped — the LAST boundary of #18):** the ship+e2e+close milestone. Three deviations from the plan sketch, all toward the simpler/more-honest design (ARCH-DRY / single-mechanism), each verified:
  - **The ship refit needs NO CV (dropped the sketched "synthesized cv-split + folds").** T18/seam-3 sketched threading a cv-split into `shapeConfigToExperiment` because `train.py` read `folds` unconditionally. But the ship refit's job is to fit the winner on ALL rows and predict — recomputing a single all-rows `cv_score` at ship would report exactly the misleading v1 resubstitution number M1a exists to kill. So instead `train.py`'s `folds` read became **conditional** (per-fold requires folds; all-rows fits-on-all and computes `cv_score` ONLY if folds are provided, for v1-plain compat). `shapeConfigToExperiment` is then correct AS-IS (`data ++ pipeline(config) ++ ship`, no cv-split). Confirmed on the real smoke run: the ship run has no `cv-split` dir and `train/metrics.json == {}`.
  - **No `{mode:all}` marker.** The plan mentioned injecting a positive `{mode:all}` fold-context at ship. The built mechanism already treats **absence of `_fold`** as fit-on-all (both `features` and `train`), so a positive marker is redundant baked-in metadata — dropped (single-mechanism principle).
  - **Zero kbench CODE changes.** The plan anticipated `kbench/steps/titanic/{features,submission}` edits. `features` already fits-on-all when `_fold` is absent and writes a captured `dataset/`; `submission` already works. So M1a-5 touched only metis (`predict.py` `exp_path`→`dataset_dir`, `train.py` folds-optional) + the shared shape declarations (already correct). The only kbench diff is a **test** (assert the ship in the smoke e2e) — still separately reviewed per the cross-repo rule.
  - **`Winner` unified** to carry the resolved `shape.Point` (dropped the redundant top-level `FreeParams`) so ship/promote rebuild from run-keys directly (T19), not by re-expanding the grid. **promote** now aggregates raw fold rows to per-config `(mean,SE)` BEFORE selecting (both `--best`/`--point`), drops the M1a-3a `HasFoldRows` refuse-guard (+ the now-consumerless helper), and records the honest `sweep_estimate` provenance. **`HonestE2E`** (`shipe2e_test.go`) proves the whole algebra + the metis#24 soundness gate end-to-end through the sweep (mutation-checked). Real 42-config Kaggle run stays operator-gated (RUNBOOK); the mechanism is proven offline (Go fake-exec + the real-step smoke sweep → a valid `submission.csv`).
