...
### 2026-07-09 — `promote --family` superseded by `metis ledger select` (M2-review finding)
Task 12 Step 2 drafted `promote --family <name>` as the offline family-grouped surface. **As shipped, the two-level family-grouped select is the new standalone `metis ledger select --rule R` command** (reusing the same pure `SelectConfigs`+exported `FamilyOf`), NOT a `promote` flag. Rationale: `ledger select` is the clean home for *selection* (choosing the robust config across the two-level rule), while `promote` stays focused on *authoring a singleton `.md`* from a chosen config; it retains raw `--best`/`--point` (an operator promotes the `ledger select` winner via `--point`). The Spec Done-when's "both surfaces group by family" is satisfied — the acceptance counterfactual runs over `ledger select`, and the `ledger select`↔in-memory DRY property is tested via identical path-qualified family keys (`TestLedgerSelect_PctLoss_PerFamilyAndShip`). No `promote --family` flag ships.
- **Known minor (deferred):** on a ledger with >1 `sweep_sha` cohort, offline `ledger select` builds `HasComplexity=false` stats for old pre-complexity rows → the guard misdiagnoses ("model reports no complexity") instead of "scope with `--sweep`". Captured in `workshop/lessons.md`; a "multiple cohorts present" warning is a follow-up (would land in #4/a ledger-hygiene issue).
