...
func cmdRun(args []string) error {
	...
	live := fs.Bool("live", false, "metis#66: fold-ordered scheduling — outer folds finish lowest-index-first (a priority queue over the leaf budget, backfill preserved so no core idles) so the mean±SE tightens fold-by-fold; press q+Enter to finalize an honest partial out<n> estimate early. Byte-identical result to the default full run (scheduling-only). DEFAULT keeps global fan-out for unattended runs; nested (multi-config) runs only.")
	autoStop := fs.Bool("auto-stop", false, "metis#66: incumbent-referenced early stop — after each outer fold (n≥2), a family whose full-k mean is <95%-likely to reach the ledger's incumbent stops its remaining outer folds (losers only; a would-be winner runs full k). Implies --live; schedules outer folds sequentially (clean per-fold decision) and marks stopped families `stopped: auto` in the ledger. Needs a prior run in the ledger for the incumbent.")
